...
void Node::compute_array_length() {
    ...
    case MDI_LIST: {
        ...
        for (auto iter = children.begin(); iter != children.end(); iter++) {
            ...
            children_length += iter->serialized_length;
        }
        // mdi_node_t is followed by list of offsets and then element nodes
        serialized_length = MDI_ALIGN(sizeof(mdi_node_t) + child_count * sizeof(mdi_offset_t)
                                      + children_length);
        break;
    }
    default:
    ...
    if (type == MDI_STRING) {
        ...
        case MDI_LIST: {
            ...
            for (auto iter = children.begin(); iter != children.end(); iter++) {
                ...
                offset += iter->serialized_length;
            }
            // recursively serialize list and array elements
            for (auto iter = children.begin(); iter != children.end(); iter++) {
                iter->serialize(out_file);
            }
            // list elements will be aligned, so no need to update pad_length
            break;
        }
        default:
            assert(0);
