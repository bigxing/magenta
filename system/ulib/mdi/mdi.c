...
mx_status_t mdi_array_uint8(const mdi_node_ref_t* ref, uint8_t index, uint8_t* out_value) {
    if (mdi_node_type(ref) != MDI_ARRAY) {
        xprintf("%s: ref not an array in mdi_array_uint8\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    const mdi_node_t* node = ref->node;
    if (MDI_ID_ARRAY_TYPE(node->id) != MDI_UINT8) {
        xprintf("%s: bad array element type for mdi_array_uint8\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    if (index >= node->value.child_count) {
        xprintf("%s: array index out of range in mdi_array_uint8\n", __FUNCTION__);
...
mx_status_t mdi_array_int32(const mdi_node_ref_t* ref, uint32_t index, int32_t* out_value) {
    if (mdi_node_type(ref) != MDI_ARRAY) {
        xprintf("%s: ref not an array in mdi_array_int32\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    const mdi_node_t* node = ref->node;
    if (MDI_ID_ARRAY_TYPE(node->id) != MDI_INT32) {
        xprintf("%s: bad array element type for mdi_array_int32\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    if (index >= node->value.child_count) {
        xprintf("%s: array index out of range in mdi_array_int32\n", __FUNCTION__);
        return ERR_OUT_OF_RANGE;
    }
    void* array_data = (char *)node + sizeof(*node);
...
mx_status_t mdi_array_uint32(const mdi_node_ref_t* ref, uint32_t index, uint32_t* out_value) {
    if (mdi_node_type(ref) != MDI_ARRAY) {
        xprintf("%s: ref not an array in mdi_array_uint32\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    const mdi_node_t* node = ref->node;
    if (MDI_ID_ARRAY_TYPE(node->id) != MDI_UINT32) {
        xprintf("%s: bad array element type for mdi_array_uint32\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    if (index >= node->value.child_count) {
        xprintf("%s: array index out of range in mdi_array_uint32\n", __FUNCTION__);
        return ERR_OUT_OF_RANGE;
    }
    void* array_data = (char *)node + sizeof(*node);
...
mx_status_t mdi_array_uint64(const mdi_node_ref_t* ref, uint32_t index, uint64_t* out_value) {
    if (mdi_node_type(ref) != MDI_ARRAY) {
        xprintf("%s: ref not an array in mdi_array_uint64\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    const mdi_node_t* node = ref->node;
    if (MDI_ID_ARRAY_TYPE(node->id) != MDI_UINT64) {
        xprintf("%s: bad array element type for mdi_array_uint64\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    if (index >= node->value.child_count) {
        xprintf("%s: array index out of range in mdi_array_uint64\n", __FUNCTION__);
        return ERR_OUT_OF_RANGE;
    }
    void* array_data = (char *)node + sizeof(*node);
...
mx_status_t mdi_array_boolean(const mdi_node_ref_t* ref, uint32_t index, bool* out_value) {
    if (mdi_node_type(ref) != MDI_ARRAY) {
        xprintf("%s: ref not an array in mdi_array_boolean\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    const mdi_node_t* node = ref->node;
    if (MDI_ID_ARRAY_TYPE(node->id) != MDI_BOOLEAN) {
        xprintf("%s: bad array element type for mdi_array_boolean\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    if (index >= node->value.child_count) {
        xprintf("%s: array index out of range in mdi_array_boolean\n", __FUNCTION__);
        return ERR_OUT_OF_RANGE;
    }
    void* array_data = (char *)node + sizeof(*node);
...
mx_status_t mdi_array_node(const mdi_node_ref_t* ref, uint32_t index, mdi_node_ref_t* out_ref) {
    ...
    if (mdi_node_type(ref) != MDI_ARRAY) {
        xprintf("%s: ref not an array in mdi_array_node\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    const mdi_node_t* node = ref->node;
    if (MDI_ID_ARRAY_TYPE(node->id) != MDI_LIST) {
        xprintf("%s: bad array element type for mdi_array_node\n", __FUNCTION__);
        return ERR_WRONG_TYPE;
    }
    if (index >= node->value.child_count) {
        xprintf("%s: array index out of range in mdi_array_node\n", __FUNCTION__);
        return ERR_OUT_OF_RANGE;
    }
    mdi_offset_t* offsets = (mdi_offset_t *)((char *)(node + 1));
