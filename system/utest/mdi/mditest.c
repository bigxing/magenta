...
bool simple_tests(void) {
    ...
    EXPECT_EQ(mdi_node_int32(&node, &i32), 0, "mdi_array_int32 failed");
...
bool array_tests(void) {
    ...
    // test boolean array
    EXPECT_EQ(mdi_list_find_node(&root, MDI_TEST_BOOL_ARRAY, &node), 0,
    ...
    // test uint8 array
    EXPECT_EQ(mdi_list_find_node(&root, MDI_TEST_UINT8_ARRAY, &node), 0,
    ...
    // test int32 array
    EXPECT_EQ(mdi_list_find_node(&root, MDI_TEST_INT32_ARRAY, &node), 0,
    ...
    // test uint32 array
    EXPECT_EQ(mdi_list_find_node(&root, MDI_TEST_UINT32_ARRAY, &node), 0,
    ...
    // test uint64 array
    EXPECT_EQ(mdi_list_find_node(&root, MDI_TEST_UINT64_ARRAY, &node), 0,
...
bool list_tests(void) {
    ...
    mdi_node_ref_t root, node, element, child;
    int32_t i32;
    const char* string;
    ...
    // test list array
    EXPECT_EQ(mdi_list_find_node(&root, MDI_TEST_LIST_ARRAY, &node), 0,
              "MDI_TEST_LIST_ARRAY not found");
    EXPECT_EQ(mdi_array_length(&node), 3u, "mdi_array_length failed");;

    for (int i = 0; i < 3; i++) {
        EXPECT_EQ(mdi_array_node(&node, i, &element), 0, "mdi_array_node failed");
        EXPECT_EQ(mdi_list_first_child(&element, &child), 0, "mdi_list_first_child failed");
        EXPECT_EQ(mdi_node_type(&child), (uint32_t)MDI_INT32, "expected type MDI_INT32");
        EXPECT_EQ(child.node->id, (uint32_t)MDI_TEST_LIST_ARRAY_INT, "expected MDI_TEST_LIST_ARRAY_INT");
        EXPECT_EQ(mdi_node_int32(&child, &i32), 0, "mdi_array_int32 failed");
        EXPECT_EQ(i32, test_ints[i], "mdi_node_int32 returned wrong value");
        EXPECT_EQ(mdi_list_next_child(&child, &child), 0, "mdi_list_next_child failed");
        EXPECT_EQ(mdi_node_type(&child), (uint32_t)MDI_STRING, "expected type MDI_STRING");
        EXPECT_EQ(child.node->id, (uint32_t)MDI_TEST_LIST_ARRAY_STR, "expected MDI_TEST_LIST_ARRAY_STR");
        string = mdi_node_string(&child);
        ASSERT_NEQ(string, NULL, "mdi_node_string returned NULL");
        EXPECT_EQ(strcmp(string, test_strings[i]), 0, "mdi_node_string failed");
        // should be end of child list
        EXPECT_NEQ(mdi_list_next_child(&child, &child), 0, "mdi_list_next_child shouldn't have succeeded");
    }
